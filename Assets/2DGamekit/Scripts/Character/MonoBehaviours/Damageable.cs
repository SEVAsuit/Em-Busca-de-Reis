...
namespace Gamekit2D
{
    public class Damageable : MonoBehaviour, IDataPersister
    {
		...
		[Serializable]
        public class DamageEventMetroidvaniaController : UnityEvent<Ally, Damageable>
        { }
		...
		public DamageEventMetroidvaniaController OnTakeDamageMetroidvaniaController;
        public DamageEvent OnDie;
		public DamageEventMetroidvaniaController OnDieMetroidvaniaController;
		...
		public void TakeDamageMetroidvaniaController(Ally ally, bool ignoreInvincible = false)
        {
            if ((m_Invulnerable && !ignoreInvincible) || m_CurrentHealth <= 0)
                return;
            ...
            if (!m_Invulnerable)
            {
                m_CurrentHealth -= ally.dmgValue;
                OnHealthSet.Invoke(this);
            }

            m_DamageDirection = transform.position + (Vector3)centreOffset - ally.transform.position;

            OnTakeDamageMetroidvaniaController.Invoke(ally, this);

            if (m_CurrentHealth <= 0)
            {
                OnDieMetroidvaniaController.Invoke(ally, this);
                m_ResetHealthOnSceneReload = true;
                EnableInvulnerability();
                if (disableOnDeath) gameObject.SetActive(false);
            }
